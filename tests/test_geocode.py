...
def test_international_address_zip_is_none():
    # France's canonical address contains '75007', but country_code 'fr' -> None.
...
def test_parse_zip_us_with_token():
    raw = {"display_name": "1, Main Street, Town, 12345, United States",
           "address": {"country_code": "us"}}
    assert geocode._parse_zip(raw) == "12345"


def test_parse_zip_non_us_returns_none():
    raw = {"display_name": "Tour Eiffel, Paris, 75007, France",
           "address": {"country_code": "fr"}}
    assert geocode._parse_zip(raw) is None


def test_parse_zip_us_without_token_returns_none():
    raw = {"display_name": "Somewhere, United States",
           "address": {"country_code": "us"}}
    assert geocode._parse_zip(raw) is None
